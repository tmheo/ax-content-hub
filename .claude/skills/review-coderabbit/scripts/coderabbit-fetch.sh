...
if [[ "$FORMAT" == "--summary" ]]; then
    ...
    echo "$CODERABBIT_THREADS" | jq '[.[] |
        ...
        select(.comments.nodes[0].path != null) |
        select(.comments.nodes[0].body | contains("Nitpick comments") | not) |
        select(.comments.nodes[0].body | contains("Actionable comments") | not) |
        {
            thread_id: .id,
            comment_id: .comments.nodes[0].databaseId,
            path: .comments.nodes[0].path,
            line: .comments.nodes[0].line,
            start_line: .comments.nodes[0].startLine,
            severity: (if .comments.nodes[0].body | contains("🔴 Critical") then "critical"
                       elif .comments.nodes[0].body | contains("🟠 Major") then "major"
                       else "minor" end),
            title: (.comments.nodes[0].body | split("\n") | map(select(contains("**") and (contains("Critical") or contains("Major") or contains("Minor") or contains("일치") or contains("오류") or contains("문제")))) | first // (.comments.nodes[0].body | split("\n")[2] | .[0:100])),
            has_suggestion: (.comments.nodes[0].body | contains("```suggestion")),
            suggestion: (if .comments.nodes[0].body | contains("```suggestion")
                        then (.comments.nodes[0].body | split("```suggestion")[1] | split("```")[0] | ltrimstr("\n") | rtrimstr("\n") // null)
                        else null end),
            has_ai_prompt: (.comments.nodes[0].body | contains("🤖 Prompt for AI Agents")),
            ai_prompt: (if .comments.nodes[0].body | contains("🤖 Prompt for AI Agents")
                       then (.comments.nodes[0].body | split("🤖 Prompt for AI Agents")[1] | split("```")[1] // null)
                       else null end),
